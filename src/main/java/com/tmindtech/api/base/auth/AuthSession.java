...
public class AuthSession {
    ...
    //pre auth key
    private static final String PRE_AUTH_KEY = "pre_auth";
    ...
    public void setPreAuth(long uid) {
        removeAuth();
        HttpSession session = getSession();
        session.setAttribute(PRE_AUTH_KEY, uid);
    }

    public long getPreAuthId() {
        try {
            Object result = getSession().getAttribute(PRE_AUTH_KEY);
            if (result == null) {
                return 0;
            }
            return (Long) result;
    ...
    /**
     * @return 优先返回AuthId，再返回PreAuthId
     */
    public long getAuthOrPreAuthId() {
        long authId = getAuthId();
        return authId != 0 ? authId : getPreAuthId();
    }
    ...
    public void removeAuth() {
        getSession().removeAttribute(PRE_AUTH_KEY);
        long uid = getAuthId();
        if (uid == 0) {
            return;
        }
